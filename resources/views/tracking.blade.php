        ...
        <div class="max-w-2xl mx-auto">
            ...
            <div class="bg-white rounded-lg shadow p-6 mb-6">
                <form id="trackingForm" class="flex gap-3">
                    <input type="text" id="orderNumber" placeholder="Masukkan nomor pesanan (contoh: WO-2026-0001)" class="flex-1 border rounded px-4 py-2">
...
<script>
    ...
    // Route untuk invoice (digenerate dari Laravel)
    ...
    document.getElementById('trackingForm').addEventListener('submit', async function(e) {
        ...
        try {
            const res = await fetch(`/tracking/${encodeURIComponent(orderNumber)}`);
            const data = await res.json();

            if (!data.order) {
                resultDiv.innerHTML = '<div class="text-red-500 text-center py-4">Pesanan tidak ditemukan</div>';
            ...
            const clothGallery = data.cloth_gallery || [];
            ...
            let html = `
                <div class="fade-enter">
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="flex justify-between"><span class="font-semibold">Status:</span> ${statusBadge[order.status] || order.status}</div>
            ...
            // Informasi Pembayaran (QRIS & Rekening)
            html += `<div class="mt-6 p-4 bg-pink-50 rounded-lg">
                        <h3 class="font-semibold text-lg mb-2">Informasi Pembayaran</h3>
                        <p class="text-sm mb-3">Silakan transfer ke rekening berikut atau scan QRIS untuk pembayaran DP atau cicilan:</p>
            ...
            // Tombol aksi: jika lunas, tampilkan download invoice; jika belum, tampilkan upload bukti (dengan sisa tagihan)
            if (order.status === 'paid' || order.status === 'completed') {
                ...
                // Kirim remaining_payment ke fungsi showUploadForm
                html += `<div class="mt-4 pt-4 border-t">
            ...
            console.error(err);
            resultDiv.innerHTML = '<div class="text-red-500 text-center py-4">Terjadi kesalahan. Pastikan nomor pesanan benar atau coba lagi nanti.</div>';
    ...
    // Fungsi upload dengan validasi tidak melebihi sisa tagihan
